/*!
 * Bank helper with select2
 *
 * @handle sharee-bank-helper
 * @deps jquery, select2
 */
...
jQuery( document ).ready( function( $ ) {
	...
	toggleSelect2();
	$( '#_bank_group_code' ).on( 'change', toggleSelect2 );

	if ( ! BankHelper.yolpKey ) {
		$( '#sharee-zip-search' ).remove();
	}
	$( '#sharee-zip-search' ).on( 'click', function( e ) {
		e.preventDefault();
		$.ajax( {
			type: 'GET',
			...
			.done( function( response ) {
				...
				$( '#_billing_address' ).val(
					response.Feature[ 0 ].Property.Address
				);
			} )
			.fail( function() {
				alert( BankHelper.yolpError );
