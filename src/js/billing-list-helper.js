 ...
 * Billing list helper
 *
 * @handle sharee-billing-list-helper
 * @deps jquery
 */
...
( ( $ ) => {
	...
	$( document ).ready( () => {
		$( '#doaction,#doaction2' ).click( function( e ) {
			...
			const ids = []; // Grab user ids.
			$( '.billing-user:checked' ).each( function( _index, input ) {
				ids.push( $( input ).val() );
			} );
			if ( ! ids.length ) {
				return false;
			...
			switch ( action ) {
				...
				case 'download':
					...
					$form
						...
						.val( $( 'select[name="year"]' ).val() );
					$form
						.find( 'input[name="month"]' )
						.val( $( 'select[name="monthnum"]' ).val() );
					...
					ids.forEach( ( id ) => {
						$form
							.find( 'p' )
							.append(
								`<input type="checkbox" name="user_ids[]" value="${ id }" checked />`
							);
					} );
					$form.submit();
					break;
